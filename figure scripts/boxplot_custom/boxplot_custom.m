...
% - 'x_position' : Vector of the "x" position of each boxplot. This option
%                  is still experimental and might cause potential 
%                  aesthetic problems in specific situations (i.e. Too many
%                  boxplots, log x-scale, mode > 1 etc.). If necessary,
%                  adjust the 'width' parameter.  This also works with
%                  horizontal direction (then it will be the y-direction).
% - 'width' : Value (1x1) of the boxplots' width. Ideally it should be less
%             than 0.5 to avoid overlapping.
%
% Output :
% - H : handles of the boxplots
%
%
% Guillaume Talbot, INRS-ETE, 2021 (updated march 2023)
%
%
% %Examples :
% X=rand(10,5);
...
% set(gca,'fontsize',10)
% title({'A group of 3 horizontal boxplots';'with the mean added'})
% 
...
% 
% %Changing the boxplots' width
% subplot(2,5,9)
% boxplot_custom(X2,list_labels,'width',0.1)
% set(gca,'fontsize',10)
% title('Smaller width')
% 
% %Changing the boxplots's position
% subplot(2,5,10)
% boxplot_custom(X3,'x_position',[1 5 6 10 18])
% set(gca,'fontsize',10)
% xlabel('X')
% title('Specify boxplots'' position')
...
if nargin>1
    ...
    for k=1:length(varargin)
        ...
        if strcmpi(varargin{k},'x_position')
            x_position=varargin{k+1};
            x_position_manual=1;
            skip=1;
            continue
        end
        ...
        if strcmpi(varargin{k},'width')
           total_width=ones(1,6).*varargin{k+1};
           skip=1;
           continue            
        end
...
if ~isempty(list_labels) && x_position_manual==1
    error('The boxplot''s positions can''t be specified at the same time as the labels')
end
...
%If x_position is not defined, used basic numerotation (1:n)
if length(Data)==1 || mode>1
    [~,nC]=size(Data(1).data);
else
    nC=length(Data);
end
if x_position_manual==0
    x_position=1:nC;
else
    %Check if the length of "x_position" is the same as the number of boxplots
    if nC~=length(x_position)
        error('The number of boxplots is not the same as the length of the vector "x_position"')
    end
end
...
for D=1:length(Data)
    ...
    for C=1:nC
        ...
        if mode==1
            xl=x_position(count)+centers(1)-width/2;
            xc=x_position(count)+centers(1);
            xr=x_position(count)+centers(1)+width/2;
            color=fillcolor{1};
        else
            xl=x_position(count)+centers(D)-width/2;
            xc=x_position(count)+centers(D);
            xr=x_position(count)+centers(D)+width/2;
            color=fillcolor{D};
        end
