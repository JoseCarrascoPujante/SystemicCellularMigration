...
% Use as h = raincloud_plot(X), where X is a data vector and h is a cell array of handles for the various figure parts.
...
addOptional(p, 'line_width', 1, @isnumeric)
...
if box_on
    ...
    if box_dodge
        ...
        % median line
        h{4} = line([Y(3) Y(3)], [((-yl(2) * box_dodge_amount) - (wdth * 0.3)) ((-yl(2) * box_dodge_amount) + (wdth * 0.3))], 'col', 'y', 'LineWidth', line_width);
        ...
        % mean line
        h{7} = line([mean(X) mean(X)], [((-yl(2) * box_dodge_amount) - (wdth * 0.3)) ((-yl(2) * box_dodge_amount) + (wdth * 0.3))], 'col', 'g', 'LineWidth', line_width);
    else
        box_pos = [Y(1) -yl(2)/2-(wdth * 0.5) Y(2)-Y(1) wdth];
        % median line
        h{4} = line([Y(3) Y(3)], [-yl(2)/2-(wdth * 0.5) -yl(2) / 2 + (wdth * 0.5)], 'col', 'y', 'LineWidth', line_width);
        ...
        % mean line
        h{7} = line([mean(X) mean(X)], [-yl(2)/2-(wdth * 0.5) -yl(2) / 2 + (wdth * 0.5)], 'col', 'g', 'LineWidth', line_width);
    end
    % 'box' of 'boxplot'
