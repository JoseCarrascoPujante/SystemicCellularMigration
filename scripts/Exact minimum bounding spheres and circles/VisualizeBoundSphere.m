...
if isempty(Tri) || size(Tri,2)~=3
    H(1)=plot3(X(:,1),X(:,2),X(:,3),'.k','MarkerSize',5);
else    
...
H(2)=trimesh(tr);
set(H(2),'EdgeColor','none','FaceColor',[0,0,1],'FaceAlpha',0.03)
H(3)=plot3(GC1(:,1),GC1(:,2),GC1(:,3),'-k','LineWidth',.5);
H(4)=plot3(GC2(:,1),GC2(:,2),GC2(:,3),'-k','LineWidth',.5);
H(5)=plot3(GC3(:,1),GC3(:,2),GC3(:,3),'-k','LineWidth',.5);
% axis tight
% axis vis3d
...
if ~isempty(Xb)
    H(8)=plot3(Xb(:,1),Xb(:,2),Xb(:,3),'.b','MarkerSize',20);
end
drawnow
pause(0.1)
