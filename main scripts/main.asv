...
for f = 1:length(UsefulSubFolderNames)
    ...
    for i = 1:length(A)
        ...
        coordinates.(conditionValidName).original_x(:,i) = tracks.(conditionValidName).(A{1})(:,1) ;
        coordinates.(conditionValidName).original_y(:,i) = tracks.(conditionValidName).(A{1})(:,2) ;
        
        % Center X and Y coordinates (substract first value to all series)
        coordinates.(conditionValidName).centered_x(:,i) = ...
            tracks.(conditionValidName).(A{1})(:,1) - tracks.(conditionValidName).(A{1})(1,1) ;
        coordinates.(conditionValidName).centered_y(:,i) = ...
            tracks.(conditionValidName).(A{1})(:,2) - tracks.(conditionValidName).(A{1})(1,2) ;
...
save(strcat(destination_folder, '\', run_date, '_coordinates'), 'N', 'stat_names', 'shuffles', 'coordinates',...
    'CoordImportAndShuffleTime') ;

['Coordinate section runtime was ' num2str(ImportTime) ' seconds']
...
for i=1:length(field_names)
    ...
    for j=N % number of trajectories per condition
        ...
        [results.(field_names{i})(j,strcmp(stat_names(:), 'alpha')),...
            results.(field_names{i})(j,strcmp(stat_names(:), 'r2')),...
            results.(field_names{i})(j,strcmp(stat_names(:), 'correlation_time')), tc2] = ...
            amebas5(coordinates.(field_names{i}).scaled_rho(:,j), rmsfhandle) ;
        
        % Shuffled RMSFalpha
        [results.(field_names{i})(j,strcmp(stat_names(:), 'shuff_alpha')),...
            results.(field_names{i})(j,strcmp(stat_names(:), 'shuff_r2')),...
            results.(field_names{i})(j,strcmp(stat_names(:), 'shuff_correlation_time')), ~] = ...
        ...
        % Shuffled DFAgamma
        [results.(field_names{i})(j,strcmp(stat_names(:), 'shuff_dfa_gamma'))] = ...
        ...
        % Shuffled MSDbeta
        [results.(field_names{i})(j,strcmp(stat_names(:), 'shuff_msd_beta'))] = ...
        ...
        results.(field_names{i})(j,strcmp(stat_names(:), 'shuff_ApEn')) = ...
...
%% File back-up and figure export
