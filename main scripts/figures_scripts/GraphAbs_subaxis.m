...
for ej=1:length(pairs)
    ...
    hold(hAxB(ej),'on')
    ellipse_gscatter(hAxB(ej),cat(2,metric1,metric2),G,conf,'m')
...
for ek=1:length(pairs)-1 % for number of small axes do...
    if ek == 2
        ...
        hold(hAxS(ek),'on')
        ellipse_scatter(hAxS(ek),cat(2,results.full(:,pairs(ek+1,1)),results.full(:,pairs(ek+1,2))),conf,'m')
    else
        scatter(hAxS(ek),results.full(:,pairs(ek+1,1)+1),results.full(:,pairs(ek+1,2)+1),.7,'y','filled','o') ;
        hold(hAxS(ek),'on')
        ellipse_scatter(hAxS(ek),cat(2,results.full(:,pairs(ek+1,1)+1),results.full(:,pairs(ek+1,2)+1)),conf,'m')
    end
    box(hAxS(ek),'on')
    xl= xlim(hAxS(ek));
    yl= ylim(hAxS(ek));
    ...
    hold(hAxB(ek+1),'on')
    rectangle(hAxB(ek+1),'Position', rPos,'EdgeColor','m','FaceColor','none','LineWidth',.5);
...
set(hAxS, 'Color','none', 'XColor','m', 'YColor','m','LineWidth',.5,'FontSize',6, ...
